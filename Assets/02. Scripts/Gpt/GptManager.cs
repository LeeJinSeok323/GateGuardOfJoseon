...
namespace OpenAi.Examples
{
    public class GptManager : MonoBehaviour
    {
        ...
        string PromptSettings(){
            ...
            NpcPrompt = $"당신은 조선시대 {Status}신분으로 {Job}직업 역할에 적합한 말투로 문지기의 질문에 대답해야 합니다. 이름은{NpcInfoManager.Name}이며 나이는 {NpcInfoManager.Age}이고 {NpcInfoManager.Hometown}출신입니다. 일상 설명:{NpcInfoManager.NpcDaily} 소지 물건:{NpcInfoManager.Item} 통행 목적:{NpcInfoManager.PassPurpose} 문지기와의 관계: 매우나쁨"; 
            return NpcPrompt;
        }
    }
}